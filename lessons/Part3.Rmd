...
⚠️When dropping missing values, it drops the entire row of data that contained those missing values. This means we lose cells that had non-missing values for other variables. Choosing whether to drop the row entirely or using functions that ignore missing values will depend on what you're trying to do in your data cleaning and analysis.
