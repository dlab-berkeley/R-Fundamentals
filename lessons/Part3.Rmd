...
Before we get started, let's load in the `tidyverse` package that we used in Part 2. We will use this today for subsetting our data and for making plots.
...
In Part 2, we learned how to subset columns in **base R**. Base R refers to all the functions and operators that you can use in R without installing any additional packages. Base R will get you a long way in data science. However, it is not always the easiest code to read and interpret. A common package that people use to perform data science in R is `tidyverse`. The `tidyverse` package includes functions that are designed to be more intuitive for doing data cleaning, visualization, and analysis.
...
⚠️ It's ok for text in red to display when you load in a package. **Warnings** are fine and don't mean anything is wrong. **Errors** are problems and mean something is wrong that needs to be fixed.
...
⚠️ When dropping missing values, it drops the entire row of data that contained those missing values. This means we lose cells that had non-missing values for other variables. Choosing whether to drop the row entirely or using functions that ignore missing values will depend on what you're trying to do in your data cleaning and analysis.
