...
📝 **Poll RFun 3-2**: Which variables contain missing values? Do any of the other variables contain missing values? Edit the code below to find out and answer in the poll.
...
We might want to drop the missing values from our data entirely. We can combine `filter()` and `is.na()` to remove rows that contain missing values.

📝 **Poll RFun 3-3:** Try out the code in the poll and choose the option(s) that drop all the rows with missing values for `gdpPercap`.
...
🔔The code above contains functions we haven't used yet. What do you think `group_by()` and `summarize()` are doing? If we instead wanted the total population for each `country` in 2007, what should we change?
